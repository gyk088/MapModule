...
export default class RootMediator {
  ...
  $$initModule(moduleData) {
    ...
    if (!this.$$modules[moduleData.module]) return console.error("no such module:", moduleData.module)

    if (this.$$currentModule.name && this.$$currentModule.name === moduleData.module) {
    ...
    if (moduleData.pushState) window.history.pushState(moduleData.state, moduleData.module, `${moduleData.path}`)
  }
  ...
  $$subscribe(module, channel, cb) {
    ...
    this.$$channels[channel].push({ context: module, callback: cb })
  }
}
